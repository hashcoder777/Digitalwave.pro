...
public:
	...
	float m_flMinBodyYaw; // 0x330 минимальная десинк дельта, обычно отрицательная деф значение -58
	float m_flMaxBodyYaw; // 0x334 максимальная десинк дельта, обычно положительная деф значение 58
	Vector m_flVelocity()
	{
	...
	float m_flGroundFriction; // 0x11c
...
public:
	...
	{
		uintptr_t m_vecViewOffset = g_pNetvars->GetOffset("DT_BasePlayer", "m_vecViewOffset[0]");
...
public:
	...
	void AccuracyPenalty()
	{
		...
		return Utils::GetVFunc<OrigFn>(this, 483)(this);//scan accuracy penalty 
	}
