...
void VectorAnglesxd(const Vector& forward, QAngle& angles)
{
	...
	{
		angles[0] = (forward[2] > 0.0f) ? 279.0f : 89.0f; // Pitch (up/down)
		angles[1] = 0.0f;  //yaw left/right
	}
	else
	{
		angles[0] = atan2(-forward[2], forward.Length2D()) * -180 / M_PI;
		angles[1] = atan2(forward[1], forward[0]) * 179 / M_PI;
...
float Autowall::GetDamage(const Vector& point)
{
	float damage = 0.f;
	Vector dst = point;
	if (!g::pLocalEntity) return damage;
	FireBulletData data(g::pLocalEntity->GetEyePosition(), g::pLocalEntity);
	data.filter.pSkip1 = g::pLocalEntity;
...
bool Autowall::SimulateFireBullet2(FireBulletData& data)
{
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
		...
		UTIL_TraceLine(data.src, end, 0x4600400B, g::pLocalEntity, &data.enter_trace);
...
bool Autowall::TraceToExit(Vector& end, C_Trace& tr, float x, float y, float z, float x2, float y2, float z2, C_Trace* trace)
{
...
bool Autowall::HandleBulletPenetration2(WeaponInfo_t* wpn_data, FireBulletData& data)
{
