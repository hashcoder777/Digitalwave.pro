...
void ESP::Arrows()
{
	...
	if (g::pLocalEntity->IsAlive() && g_pEngine->IsInGame() && g_pEngine->IsConnected()) {
		...
		const auto realangle = g::pLocalEntity->AnimState()->m_flGoalFeetYaw;
	...
	//		g_pSurface->DrawT(10, screeny - 70, Color(255, 0, 0, 255), indicatorfont, false, xorstr("BAIM").crypt_get());
		...
		g_AntiAim.invertsyka ? g_pSurface->DrawT(10, screeny - 70, Color(0, 255, 0, 255), indicatorfont, false, "LEFT") : g_pSurface->DrawT(10, screeny - 70, Color(0, 255, 0, 255), indicatorfont, false, "RIGHT");
		auto NetChannel = g_pEngine->GetNetChannel();
		if (g_Menu.Config.FakeLagEnable)
		{
			g_pSurface->DrawT(10, screeny - 275, NetChannel->m_nChokedPackets ? Color(0, 255, 0, 255) : Color(255, 0, 0, 255), g::IndicatorFont, false, "LAG");
			g_pSurface->FilledRect(70, screeny - 275, NetChannel->m_nChokedPackets * 5, 20, Color(255, 255, 255, 255));
		}
		if (GetAsyncKeyState(g_Menu.Config.FakeduckKey))
		{
			g_pSurface->DrawT(10, screeny - 250, Color(255, 255, 255, 255), g::IndicatorFont, false, "DUCK");
		}
		if (g_Menu.Config.shotcounter)
		{
			if (g::pLocalEntity->IsAlive())
			{
				...
				g_pSurface->DrawT(55,screeny - 400, Color(255, 255, 255), g::IndicatorFont, true, shotscounter.c_str());
				g_pSurface->DrawT(55,screeny - 430, Color(255, 255, 255), g::IndicatorFont, true, shotscounter2.c_str());
				g_pSurface->DrawT(55,screeny - 460, Color(255, 255, 255), g::IndicatorFont, true, shotscounter3.c_str());

			}
		}
		if (g::lcBroken)
		{
			g_pSurface->DrawT(10, screeny - 225, Color(0, 255, 0, 255), g::IndicatorFont, false, "BREAK LC");
		}
		...
		if (g_Menu.Config.Arrows) {
			if (g_Misc.isthirdperson) {
				...
				g_pSurface->DrawT(screen2.x, screen2.y, Color(244, 100, 66, 255), niggafont, false, "FAKE");
...
void ESP::watermark() // who needs gettextsize LOL
{
	...
	ss << "digitalwave" << " | incoming: " << incoming.c_str() << "ms" << " | outgoing: " << outgoing.c_str() << "ms" << " | fps: " << fps;

	RECT TextSize = g_pSurface->GetTextSizeRect(supremacy, ss.str().c_str());
	...
	g_pSurface->DrawT(width - (TextSize.right + 15), 8, Color(200, 200, 200), supremacy, false, ss.str().c_str());
/*	int width, height;
	g_pEngine->GetScreenSize(width, height);
	...
	ss << "digitalwave | rtt: " << outgoing.c_str() << " | rate: " << cl_cmdrate->GetFloat() << " | " << buf;
	...
	g_pSurface->FilledRect(width - x - z, 4, y + z - 1, 20, Color(0, 0, 0, 170));
