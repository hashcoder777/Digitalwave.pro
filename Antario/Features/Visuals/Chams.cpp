...
#include "../../Hooks.h"
#include "..\Aimbot\ResolverRecode.h"
...
void Chams::BacktrackChams(void* ecx, void* edx, IMatRenderContext* context, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* matrix)
{
	...
	if (g_Menu.Config.FakeLagModel)
	{
		...
		if (pPlayerEntity && pPlayerEntity->IsAlive() && !pPlayerEntity->IsDormant() && g_Aimbot.Matrix[info.index] && strstr(mdl->szName, "models/player") && g_Menu.Config.FakeLagModel) {
	...
	void* ecx, void* edx, IMatRenderContext* context, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* matrix)
{
	...
	static IMaterial* DesyncMaterial = nullptr;
	...
	else if (g_Menu.Config.ChamsMaterial == 8) {
		Material = g_pMaterialSys->FindMaterial("dev/glow_armsrace", TEXTURE_GROUP_MODEL);
	}
	...
	else if (g_Menu.Config.HandChamsMaterial == 8) {
		HandMaterial = g_pMaterialSys->FindMaterial("dev/glow_armsrace", TEXTURE_GROUP_MODEL);
		static auto pVar = g_pCvar->FindVar("$envmaptint");
	}
		...
		if (ent && g::pLocalEntity && ent->IsAlive()) {
			if (ent == g::pLocalEntity) {
				if (g_Menu.Config.LocalChams) {
					...
					Material->AlphaModulate(g_Menu.Config.LocalChamsColor.a / 255.f);// краш
